...
## There is a problem with auto-detecting hdd in usb adapters, disabled and hard coded the disk references
#device_list="$(/usr/sbin/smartctl --scan-open | awk '/^\/dev/{print $1 "|" $3}')"
device_list="/dev/sda2|sat
/dev/sdb1|sat
/dev/sdc1|sat
/dev/sdd1|sat
"
...
for device in ${device_list}; do
  # Wake drive if sleeping, using smartctl query
